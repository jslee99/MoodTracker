...
class CalendarViewAdapter(var DiaryList : ArrayList<Diary>) : RecyclerView.Adapter<CalendarViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.binding.apply {
            //날짜 설정, 선택된 Month에 day가 없으면 date가 -1
            ...
            if(DiaryList[position].mood > -1){
                when(DiaryList[position].mood){
                    0 -> moodImage.setImageResource(R.drawable.sohappy_0)
                    1 -> moodImage.setImageResource(R.drawable.happy_1)
                    2 -> moodImage.setImageResource(R.drawable.ok_2)
                    3 -> moodImage.setImageResource(R.drawable.angry_3)
                    4 -> moodImage.setImageResource(R.drawable.sad_4)
                }
            }
        }
    }
