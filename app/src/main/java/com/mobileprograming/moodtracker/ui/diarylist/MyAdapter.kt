...
class MyAdapter (val diaryList:List<Diary>)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.binding.diaryContent.text=diaryList[position].content
        ...
        if (imagebyte != null) {
            holder.binding.diaryImage.setImageBitmap(BitmapFactory.decodeByteArray(imagebyte,0,imagebyte.size))
        }
    }
